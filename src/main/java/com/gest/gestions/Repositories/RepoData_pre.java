...
public interface RepoData_pre extends JpaRepository<Data_pre,Long> {
    ...
    @Query(value="SELECT * FROM data_pre where data_pre.id=(SELECT MAX(data_pre.id) as maxi FROM data_pre where data_pre.patient_id=?)",nativeQuery = true)
    Data_pre findLastUser(Long id);
